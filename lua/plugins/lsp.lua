...
--[[
Использовать если ошибка не помещается на экран (курсор должен быть на строке с ошибкой)
Чтобы попасть внутрь окна с полной ошибкой (например, для копирования) нужно нажать сочетание клавиш ещё раз
--]]
...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(ev)
    ...
    vim.keymap.set('n', '<space>rn', vim.lsp.buf.rename, opts) -- Переименовать что-то во всём активном файле (курсор должен быть на нужном "слове")
